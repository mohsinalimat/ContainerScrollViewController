...
A common UIKit Auto Layout task involves creating a view controller with a layout that is too large to fit older, smaller devices, or devices in landscape orientation, or the area of the screen above the keyboard.

For example, consider this sign up screen, which fits on iPhone XS, but not on iPhone SE when the keyboard is presented:

<img src="Images/Purpose-Comparison.png" width="888px" align="center">

This case can be handled by nesting the view inside a scroll view. This can be done manually in Interface Builder, as described by Apple's [Working with Scroll Views](https://developer.apple.com/library/archive/documentation/UserExperience/Conceptual/AutolayoutPG/WorkingwithScrollViews.html) documentation, but this approach can be awkward. According to the Medium article [How to configure a UIScrollView with Auto Layout in Interface Builder](https://medium.com/@pradeep_chauhan/how-to-configure-a-uiscrollview-with-auto-layout-in-interface-builder-218dcb4022d7), 17 steps are required.
